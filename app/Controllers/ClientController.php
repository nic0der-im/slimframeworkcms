...
class ClientController extends Controller {
	...
	public function getVer($request,$response,$args){
		...
		$pagos = Comprobante::where('estado',1)->where('id_cliente',$id_cliente)->whereIn('id',$recibos)->where('tipo',1)->distinct('id');
		$ventas = Comprobante::where('estado',1)->where('id_cliente',$id_cliente)->whereNotIn('id',$pagos->pluck('id')->toArray())->where('tipo',1)->distinct('id');

		$comprobantes = Comprobante::where('id_cliente',$id_cliente)
			->get();
		$cuentas = Cuenta::where('id_provedor_cliente',$id_cliente)->where('tipo_cuenta',1)->get();

		$dateros = Datero::where('id_cliente',$id_cliente)->orWhere('id_cliente_conyugue',$id_cliente)->get();

		return $this->container->view->render($response, 'admin_views/clientes/verCliente.twig',[
			'return_to'=>$return_to,
			'tipoResponsable' => TipoResponsable::where('estado',1)->get(),
			'cliente' => $cliente,
